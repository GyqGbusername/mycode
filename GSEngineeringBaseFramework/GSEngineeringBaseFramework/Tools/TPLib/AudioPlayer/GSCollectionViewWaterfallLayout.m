...
#import "GSCollectionViewWaterfallLayout.h"
#import "tgmath.h"

NSString *const GSCollectionElementKindSectionHeader = @"GSCollectionElementKindSectionHeader";
NSString *const GSCollectionElementKindSectionFooter = @"GSCollectionElementKindSectionFooter";

@interface GSCollectionViewWaterfallLayout ()
/// The delegate will point to collection view's delegate automatically.
@property (nonatomic, weak) id <GSCollectionViewDelegateWaterfallLayout> delegate;
...
@implementation GSCollectionViewWaterfallLayout

/// How many items to be union into a single rectangle
static const NSInteger unionSize = 20;

static CGFloat GSFloorCGFloat(CGFloat value) {
...
- (void)setItemRenderDirection:(GSCollectionViewWaterfallLayoutItemRenderDirection)itemRenderDirection {
...
- (CGFloat)itemWidthInSectionAtIndex:(NSInteger)section {
  ...
  return GSFloorCGFloat((width - (columnCount - 1) * columnSpacing) / columnCount);
}
...
- (id <GSCollectionViewDelegateWaterfallLayout> )delegate {
  return (id <GSCollectionViewDelegateWaterfallLayout> )self.collectionView.delegate;
}
...
- (void)commonInit {
  ...
  _itemRenderDirection = GSCollectionViewWaterfallLayoutItemRenderDirectionShortestFirst;
}
...
- (void)prepareLayout {
  ...
  NSAssert([self.delegate conformsToProtocol:@protocol(GSCollectionViewDelegateWaterfallLayout)], @"UICollectionView's delegate should conform to GSCollectionViewDelegateWaterfallLayout protocol");
  ...
  for (NSInteger section = 0; section < numberOfSections; ++section) {
    ...
    CGFloat width = self.collectionView.bounds.size.width - sectionInset.left - sectionInset.right;
    NSInteger columnCount = [self columnCountForSection:section];
    CGFloat itemWidth = GSFloorCGFloat((width - (columnCount - 1) * columnSpacing) / columnCount);
//      CGFloat itemWidth = GSFloorCGFloat((width - (columnCount + 1) * columnSpacing) / columnCount);//修改collection的布局
    ...
    if (headerHeight > 0) {
      attributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:GSCollectionElementKindSectionHeader withIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
    ...
    for (idx = 0; idx < itemCount; idx++) {
      ...
      if (itemSize.height > 0 && itemSize.width > 0) {
        itemHeight = GSFloorCGFloat(itemSize.height * itemWidth / itemSize.width);
      }
    ...
    if (footerHeight > 0) {
      attributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:GSCollectionElementKindSectionFooter withIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
  ...
  if ([kind isEqualToString:GSCollectionElementKindSectionHeader]) {
    attribute = self.headersAttribute[@(indexPath.section)];
  } else if ([kind isEqualToString:GSCollectionElementKindSectionFooter]) {
...
- (NSUInteger)nextColumnIndexForItem:(NSInteger)item inSection:(NSInteger)section {
  ...
  switch (self.itemRenderDirection) {
    case GSCollectionViewWaterfallLayoutItemRenderDirectionShortestFirst:
      index = [self shortestColumnIndexInSection:section];
      break;

    case GSCollectionViewWaterfallLayoutItemRenderDirectionLeftToRight:
      index = (item % columnCount);
      break;

    case GSCollectionViewWaterfallLayoutItemRenderDirectionRightToLeft:
